...
let copy =(arr)=> {let a = [...arr]; return a}    
...
let is31Flavors =(arr)=> arr.length == 31 ?  true : false
...
let addFlavor =(arr, str)=> {let a = [str,...arr]; return a}
...
let removeLastFlavor=(arr)=> {arr.pop(); return arr}
...
let getFlavorByIndex=(arr,i)=> {return arr[i].toString()}
...
let removeFlavorByName=(arr,str)=> {
  let e = [...arr];
  let i = e.indexOf(str);
   e.splice(i,1)
   return e
...
let filterByWord=(arr,str)=> {
  let a = []
  for(let i = 0; i < arr.length; i++){
    arr[i].includes(str) ? a.push(arr[i]) : console.log(a)
  }
  return a 
}
